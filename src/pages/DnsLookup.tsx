...
const DNS_TYPE_NAMES: Record<string, string> = {
  ...
  PTR: 'Pointer Record', SRV: 'Service Record', CAA: 'Certificate Authority', DS: 'Delegation Signer', DNSKEY: 'DNS Key',
};
...
const DnsLookup = () => {
  ...
  const handleSubmit = async (query: string, recordType?: string) => {
    ...
    try {
      ...
      const [response, dnssec] = await Promise.all([
        queryDNS(query, type),
        checkDNSSEC(query),
      ]);
          ...
          <ResultsTable
            ...
            items={
              response?.Answer?.length
                ...
                : [{ label: type, value: 'No records found', status: 'warning' as const }]
            }
          />
          <ResultsTable
  ...
  return (
    <ToolPageLayout
      icon={Globe}
      title="DNS Lookup"
      description="Query DNS records for any domain. Select a record type to retrieve specific DNS information."
